...
from .blocks.learned_fusion_3d import LearnedFusion3D as LearnedFusion
from .blocks.robustmvd_3D_context_encoder_bnrelu import (
    CostVolume3DContextEncoderBNRelu as CostVolume3DContextEncoder,
)
...
class RobustMVDGroupWiseCorr(nn.Module):
    def __init__(self):
        ...
        self.fusion_enc_block = DispnetCostvolumeEncoder(
            in_channels=self.num_sampling_points
        )
    ...
    def forward(self, images, poses, intrinsics, keyview_idx, **_):
        ...
        corrs_groupwise, masks_groupwise, _ = self.corr_block_groupwise(
            feat_key=enc_key,
        ...
        fused_corr, _ = self.fusion_block(corrs=corrs_groupwise, masks=masks_groupwise)
        del enc_key, corrs_groupwise, masks_groupwise
        print(f"fused_corr: {fused_corr.shape}") if verbose else None
        fused_corr_context = self.costvolume_context_encoder(fused_corr)
        fused_corr_context = fused_corr_context.squeeze(1)
        all_enc_fused, enc_fused = self.fusion_enc_block(
            corr=fused_corr_context, ctx=ctx
        )
        del fused_corr, ctx
