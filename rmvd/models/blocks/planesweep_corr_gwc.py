...
class TorchCorr(nn.Module):  # based on RAFT
    ...
    def forward(self, feat_ref, feat_src, grids=None, mask=None):
        ...
        corr = build_gwc_volume(feat_ref, feat_src, S, 32)
        del feat_ref, feat_src
        # corr = torch.matmul(
        #     feat_ref.transpose(1, 2), feat_src
        # )  # N, h_ref*w_ref, C @ N, C, h_src*w_src -> N, h_ref*w_ref, h_src*w_src
        K2HW_shape = [N * S, 2, grids.shape[3], grids.shape[4]]
        ...
        NSCHW_shape = corr.shape
        corr = corr.reshape(N * S, NSCHW_shape[2], h_src, w_src)
        # grids = grids.permute(0, 3, 4, 2, 1)  # N, h_ref, w_ref, 2, S
        # grids = grids.reshape(N * h_ref * w_ref, 2, S).unsqueeze_(3)  # K, 2, S, 1
        grids = torch.reshape(grids, K2HW_shape)
        corr, corr_mask = warp(
        ...
        #     0, 3, 1, 2
        # )  # N, S, h_ref, w_ref
        corr = torch.reshape(corr, NSCHW_shape)
        corr_mask = torch.reshape(corr_mask, [N, S, h_ref, w_ref])
...
class PlanesweepCorrelationGWC(nn.Module):
    @torch.no_grad()
