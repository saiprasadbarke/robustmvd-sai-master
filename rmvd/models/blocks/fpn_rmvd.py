...
class FeaturePyramidNetRMVD(nn.Module):
    """
    output 3 levels of features using a FPN structure
    """

    def __init__(self, use="solo"):
        super(FeaturePyramidNetRMVD, self).__init__()
        self.use = use
    ...
    def forward(self, x):
        ...
        feat1 = self._upsample_add(feat2, lat_1)  # (B, 256, H//4, W//4)
        print("fpn_feat1.shape: ", feat1.shape) if verbose else None
        feat0 = self._upsample_add(feat1, lat_0)  # (B, 256, H//2, W//2)
        print("fpn_feat0.shape: ", feat0.shape) if verbose else None

        # reduce output channels
        feat1 = self.smooth1(feat1)  # (B, 128, H//4, W//4)
        print("fpn_smooth_feat1.shape: ", feat1.shape) if verbose else None
        feat0 = self.smooth0(feat0)  # (B, 64, H//2, W//2)
        print("fpn_smooth_feat0.shape: ", feat0.shape) if verbose else None

        if self.use == "solo":
            feats = {"conv1": feat0, "conv2": feat1, "conv3a": feat2}
            return feats, feat2
        elif self.use == "cascade":
