...
class MVSNet_DINO(nn.Module):
    def __init__(self):
        ...
        self.decoder_vit = VITDecoderStage4Single()
    ...
    def forward(self, images, poses, intrinsics, keyview_idx, depth_range=None, **_):
        ...
        normalize_half_vit = transforms.Normalize(
            mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
        )
        image_key = normalize_half_vit(image_key)
        images_source = [
            normalize_half_vit(image_source) for image_source in images_source
        ]
        ...
        add_feat_key = enc_key + dino_feat_key_attn
        add_feat_sources = [
            enc_source + dino_feat_source_attn
        ...
        corrs, masks, sampling_invdepths = self.corr_block(
            feat_key=add_feat_key,
            intrinsics_key=intrinsics_key,
            feat_sources=add_feat_sources,
        ...
        del add_feat_sources
        fused_corr, _ = self.fusion_block(
            feat_key=add_feat_key, corrs=corrs, masks=masks
        )
        print(f"fused_corr: {fused_corr.shape}") if verbose else None
        del corrs, masks, add_feat_key
