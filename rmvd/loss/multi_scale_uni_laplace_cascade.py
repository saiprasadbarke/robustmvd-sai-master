...
class MultiScaleUniLaplace(nn.Module):
    def __init__(
        ...
        self.cas_level_weights = [0.5, 1, 2]
    ...
    def forward(self, sample_inputs, sample_gt, pred, aux, iteration):
        ...
        for level_cascade, (
            preds_current_level,
            pred_log_bs_current_level,
        ) in enumerate(zip(preds_all, pred_log_bs_all)):
            for level, (pred, pred_log_b) in enumerate(
                zip(preds_current_level, pred_log_bs_current_level)
            ):
                with torch.no_grad():
                ...
                if iteration < self.deterministic_loss_iterations:
                    loss = mae(
                        ...
                        * self.cas_level_weights[level_cascade],
                    )
...
@register_loss
def robust_mvd_cascade_loss(**kwargs):
